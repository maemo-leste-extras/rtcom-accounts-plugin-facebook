...
#define FACEBOOK_NEW_ACCOUNT_URI \
  "https://www.facebook.com/signup?locale2="

typedef struct _FacebookPluginClass FacebookPluginClass;
typedef struct _FacebookPlugin FacebookPlugin;

struct _FacebookPlugin
{
  RtcomAccountPlugin parent_instance;
};

struct _FacebookPluginClass
{
  RtcomAccountPluginClass parent_class;
};

ACCOUNT_DEFINE_PLUGIN(FacebookPlugin, facebook_plugin,
                      RTCOM_TYPE_ACCOUNT_PLUGIN);

static void
facebook_plugin_init(FacebookPlugin *self)
{
  RtcomAccountService *service;

  RTCOM_ACCOUNT_PLUGIN(self)->name = "facebook";
  ...
  g_object_set(G_OBJECT(service),
               "display-name", "Facebook Messenger",
               NULL);
...
create_advanced_settings_page(RtcomDialogContext *context)
{
  ...
  if (!dialog)
  {
    ...
    gchar title[200];
    const gchar *msg;
    GladeXML *xml = glade_xml_new(PLUGIN_XML_DIR "/facebook-advanced.glade",
...
static void
facebook_plugin_on_advanced_cb(gpointer data)
{
  GtkWidget *dialog;
...
static void
facebook_plugin_on_register_cb(gpointer userdata)
{
  gchar *uri, *lang, *p;
  GError *error = NULL;
  ...
  uri = g_strconcat(FACEBOOK_NEW_ACCOUNT_URI, lang, NULL);
  g_free(lang);
...
static void
facebook_plugin_context_init(RtcomAccountPlugin *plugin,
                             RtcomDialogContext *context)
{
  gboolean editing;
  AccountItem *account;
  ...
  if (editing)
  {
    ...
    rtcom_edit_connect_on_advanced(
          RTCOM_EDIT(page), G_CALLBACK(facebook_plugin_on_advanced_cb),
          context);
  }
  else
  {
    ...
    rtcom_login_connect_on_register(
      RTCOM_LOGIN(page), G_CALLBACK(facebook_plugin_on_register_cb), NULL);
    rtcom_login_connect_on_advanced(
          RTCOM_LOGIN(page), G_CALLBACK(facebook_plugin_on_advanced_cb),
          context);
  }
...
static void
facebook_plugin_class_init(FacebookPluginClass *klass)
{
  RTCOM_ACCOUNT_PLUGIN_CLASS(klass)->context_init = facebook_plugin_context_init;
}
